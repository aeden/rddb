...
class BasicMaterializerTest < Test::Unit::TestCase
  def test_initialization
    materializer = Rddb::Materializer::BasicMaterializer.new(:database)
  ...
  def test_materialize
    ...
    assert_nil v.materialized
    materializer = Rddb::Materializer::BasicMaterializer.new(database)
    materializer.materialize(v)
    assert_equal 2, v.materialized.length
  end
  
  def test_document_added
    database(:materializer_class => Rddb::Materializer::BasicMaterializer)
    v = database.views['test']
    assert_nil v.materialized
    database << {:name => 'Chris'}
